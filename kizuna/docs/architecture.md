...
```
1. /oauth/google/start: require KAO_URL + KAO_TOKEN; 302 to
...
- **Google identity is delegated to Kao.** Kizuna does not own a Google refresh token, an encryption key, or a CSRF secret — all three live in Kao (`kao/apps/api/src/lib/{encryption,oauth-state}.ts`). The only Google-related credential here is `KAO_TOKEN`, the bearer that gates `${KAO_URL}/grants/kizuna/token`. Kizuna's `/oauth/google/start` is a 302 to `${KAO_URL}/oauth/kizuna/start`; `/oauth/google/status` is a server-side proxy that reshapes Kao's grant row to the legacy `OAuthStatus` envelope.
...
- **Logging.** `apps/api/src/lib/logger.ts` is a thin wrapper around `@kagami/logger`'s `createLogger`, which provides the stable `service`/`component`/`env` bindings and the common secret-redaction list. Pretty transport whenever `NODE_ENV !== "production"`. Used directly in workers / boot / errors; there's no request-scoped logger middleware today.
