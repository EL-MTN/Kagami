...
- **Pino logging** — singleton in `apps/api/src/lib/logger.ts`, built from the workspace-shared `@kagami/logger` factory (stable `service`/`component`/`env` bindings; an `error`-key serializer that preserves stacks; **no secret/PII redaction** — removed, local-trust only); `pino-pretty` whenever `NODE_ENV !== "production"`. `logger.info({ context }, "message")` pattern. When `KANSOKU_URL` and `KANSOKU_INGEST_TOKEN` are set, logs also stream to the workspace's Kansoku service via a fail-open in-process shipper. There is no `pino-http` middleware today; route logging is request-scoped only via thrown errors hitting `makeErrorHandler`.
