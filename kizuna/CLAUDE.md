...
- **Pino logging** — singleton in `apps/api/src/lib/logger.ts`, built from the workspace-shared `@kagami/logger` factory (stable `service`/`component`/`env` bindings + common secret redaction); `pino-pretty` whenever `NODE_ENV !== "production"`. `logger.info({ context }, "message")` pattern. There is no `pino-http` middleware today; route logging is request-scoped only via thrown errors hitting `makeErrorHandler`.
