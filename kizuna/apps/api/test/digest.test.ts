...
describe("GET /v1/digest", () => {
  ...
  it("returns empty arrays when nothing is open", async () => {
    const res = await request(h.app).get("/v1/digest").set("authorization", auth());
  ...
  it("respects ?window=P30D", async () => {
    ...
    const res = await request(h.app).get("/v1/digest?window=P30D").set("authorization", auth());
    expect(res.body.upcoming.map((f: { reason: string }) => f.reason)).toEqual(["distant"]);
  });

  it('accepts the short "7d" form', async () => {
    const res = await request(h.app).get("/v1/digest?window=7d").set("authorization", auth());
    expect(res.status).toBe(200);
  ...
  it("rejects an invalid window with 400", async () => {
    const res = await request(h.app).get("/v1/digest?window=garbage").set("authorization", auth());
