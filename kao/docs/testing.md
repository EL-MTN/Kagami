...
| `config.test.ts`         | valid env + defaults; missing Google id, bad key length, short bearer, non-mongodb URI all rejected; `callbackUrl` trailing-slash strip                                                                                                                                                                                                                    |
| `routes.test.ts`         | supertest over the real app on in-memory Mongo: `/healthz` + `/` + `/oauth/:grant/start` (302, registry scopes, **no `gmail.send` for kizuna**) open; unknown grant 404; callback missing/bad-state 400/401; **`/grants` 401 without/with-wrong bearer, 200 with bearer**; unknown grant 404; `/grants/:grant/token` → 409 `no_grant`; `DELETE` idempotent |
