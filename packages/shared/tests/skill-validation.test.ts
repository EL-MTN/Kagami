...
import { computeNextRunAt, validateCronAndDefaults } from "../src/skill-validation";
...
describe("validateCronAndDefaults", () => {
  ...
  it("returns missing-defaults listing every required param without a default", () => {
    ...
    expect(result).toEqual({
      ...
      missing: ["topic", "limit"],
      message:
        "Cron-scheduled skills require defaults for all required parameters. Missing: topic, limit",
    });
  });
