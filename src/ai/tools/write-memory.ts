...
export const writeMemory = tool({
  ...
  parameters: z.object({
    path: z
      .string()
      .describe(
        "Path relative to vault root, e.g. 'memories/about-you.md'",
      ),
    content: z.string().describe("Only NEW bullet points or lines to add. Do not repeat existing content."),
    mode: z
      .enum(["append", "overwrite"])
      .default("append")
      .describe("'append' adds new lines (deduped). 'overwrite' replaces the whole file — use only for reorganizing."),
  }),
