...
describe('GET /v1/digest', () => {
  ...
  it('partitions followups into overdue vs upcoming inside the default window', async () => {
    ...
    const res = await request(h.app).get('/v1/digest').set('authorization', auth());
    expect(res.status).toBe(200);
    expect(res.body.overdue.map((f: { reason: string }) => f.reason)).toEqual([
      'send the deck',
    ]);
    expect(res.body.upcoming.map((f: { reason: string }) => f.reason)).toEqual([
      'lunch',
    ]);
  });
