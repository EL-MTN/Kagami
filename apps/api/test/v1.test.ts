...
const del = (p: string) =>
  request(h.app).delete(p).set('authorization', auth());
...
describe('interactions + recordInteraction', () => {
  it('POST inserts and $max-updates lastInteractionAt on participants', async () => {
    ...
    expect(new Date(ar.body.lastInteractionAt).toISOString()).toBe(
      new Date(t1).toISOString(),
    );
    expect(new Date(br.body.lastInteractionAt).toISOString()).toBe(
      new Date(t1).toISOString(),
    );
  });
  ...
  it('does not roll back lastInteractionAt on an earlier interaction', async () => {
    ...
    await post('/v1/interactions', {
      occurredAt: t1, channel: 'email', title: 'one',
      participants: [{ personId: a.body.id, role: 'from' }],
    });
    await post('/v1/interactions', {
      occurredAt: t0, channel: 'email', title: 'earlier',
      participants: [{ personId: a.body.id, role: 'from' }],
    });
    let r = await get(`/v1/people/${a.body.id}`);
    expect(new Date(r.body.lastInteractionAt).toISOString()).toBe(
      new Date(t1).toISOString(),
    );

    await post('/v1/interactions', {
      occurredAt: t2, channel: 'email', title: 'later',
      participants: [{ personId: a.body.id, role: 'from' }],
    });
  ...
  it('list filters by orgId via participant→primaryOrg join', async () => {
    ...
    const a = await post('/v1/people', {
      displayName: 'A', primaryOrgId: org.body.id,
    });
    const b = await post('/v1/people', { displayName: 'B' });
    await post('/v1/interactions', {
      occurredAt: '2026-01-01T00:00:00Z', channel: 'email', title: 'A-msg',
      participants: [{ personId: a.body.id, role: 'from' }],
    });
    await post('/v1/interactions', {
      occurredAt: '2026-01-02T00:00:00Z', channel: 'email', title: 'B-msg',
      participants: [{ personId: b.body.id, role: 'from' }],
    });
  ...
  it('list filters by status (default active hides cancelled)', async () => {
    ...
    const i1 = await post('/v1/interactions', {
      occurredAt: '2026-01-01T00:00:00Z', channel: 'calendar', title: 'live',
      participants: [{ personId: a.body.id, role: 'attendee' }],
    });
  ...
  it('GET /v1/people/:id/interactions filters to that person', async () => {
    ...
    const b = await post('/v1/people', { displayName: 'B' });
    await post('/v1/interactions', {
      occurredAt: '2026-01-01T00:00:00Z', channel: 'email', title: 'mine',
      participants: [{ personId: a.body.id, role: 'from' }],
    });
    await post('/v1/interactions', {
      occurredAt: '2026-01-02T00:00:00Z', channel: 'email', title: 'theirs',
      participants: [{ personId: b.body.id, role: 'from' }],
    });
  ...
  it('DELETE tombstones and excludes from default list', async () => {
    ...
    const i = await post('/v1/interactions', {
      occurredAt: '2026-01-01T00:00:00Z', channel: 'email', title: 'x',
      participants: [{ personId: a.body.id, role: 'from' }],
    });
...
describe('followups CRUD', () => {
  ...
  it('PATCH transitions status', async () => {
    ...
    const f = await post('/v1/followups', {
      personId: a.body.id, direction: 'they_owe', reason: 'await reply',
    });
  ...
  it('filters by direction', async () => {
    const a = await post('/v1/people', { displayName: 'A' });
    await post('/v1/followups', {
      personId: a.body.id, direction: 'i_owe', reason: 'r1',
    });
    await post('/v1/followups', {
      personId: a.body.id, direction: 'they_owe', reason: 'r2',
    });
  ...
  it('DELETE tombstones', async () => {
    const a = await post('/v1/people', { displayName: 'A' });
    const f = await post('/v1/followups', {
      personId: a.body.id, direction: 'i_owe', reason: 'r',
    });
...
describe('manifest', () => {
  it('GET /v1/_manifest returns endpoints + JSON Schemas', async () => {
    ...
    const addPerson = (r.body.endpoints as Array<{
