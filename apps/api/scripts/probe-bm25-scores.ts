...
function spawnWorker(itemPath: string, outPath: string, dbName: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const child = spawn(
      'npx',
      ['tsx', path.resolve(__dirname, 'probe-bm25-scores.ts')],
      {
...
async function orchestrate(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `     terms=${sample.query_term_count} facts=${sample.fact_count} ` +
          `top=${(sample.scores[0] ?? 0).toFixed(3)} median=${(quantile(sample.scores.slice().sort((x, y) => y - x).reverse(), 0.5)).toFixed(3)}`,
      );
    } catch (err) {
  ...
  console.log(
    'bucket   queries  samples   median    p75    p90    max  | old_mp/sp  -> proposed',
  );
  ...
  const outPath = path.join(benchRoot, `bm25-probe-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
  await fs.writeFile(
    outPath,
    JSON.stringify({ stats, samples }, null, 2),
  );
  console.log('');
