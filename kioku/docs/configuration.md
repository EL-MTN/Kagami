...
| `KIOKU_TIMEZONE`                   | no       | —                                                       | Operator timezone (IANA name) for day-granular `event_date` stamping — localToday()/sessionDateOf() in src/dates.ts resolve calendar days in this zone.                                                                                           |
...
| `KIOKU_CONSOLIDATE_ENABLED`        | no       | `false`                                                 | Enable the in-process periodic consolidation cron: a durable-only entity-grouped curation pass (DROPS episodic chat-exhaust, MERGES fragmented episodes — see prompts/consolidate.md) that runs to convergence over the default scope on a timer. |
| `KIOKU_CONSOLIDATE_INTERVAL_HOURS` | no       | `24`                                                    | Hours between consolidation cron passes (fractional allowed for testing).                                                                                                                                                                         |
| `KIOKU_CONSOLIDATE_MODEL`          | no       | —                                                       | Editor model for the consolidation cron, overriding LLM_MODEL for that pass only (the answerer keeps LLM_MODEL).                                                                                                                                  |
