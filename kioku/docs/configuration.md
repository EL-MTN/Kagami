...
| `KIOKU_CONSOLIDATE_ENABLED`        | no       | `false`                                                 | Enable the in-process periodic consolidation cron: a single durable-only entity-grouped curation pass (DROPS episodic chat-exhaust, MERGES fragmented episodes — see prompts/consolidate.md) over the default scope on a timer. |
