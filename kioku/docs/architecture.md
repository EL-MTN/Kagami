...
- **Periodic durable-only consolidation (in-process).** Beyond the operator-run curation pass (`ingest/curate.ts` via `scripts/curate.ts`), an optional in-process timer (`maintenance/scheduler.ts`, gated on `KIOKU_CONSOLIDATE_ENABLED`, **default off**) runs the entity-grouped durable-only pass (`prompts/consolidate.md`) **to convergence** over the default scope on an interval — DROPPING episodic chat-exhaust and folding a fragmented episode into one durable fact, repeating until the store stops changing so cross-group duplicates the first entity pass leaves get merged. Same `history`-journaled mutation path as the curation pass (actor `consolidate-cron`); merge categories are clamped to the fixed enum. A guarded `POST /consolidate` triggers one pass manually when enabled. The editor model is `KIOKU_CONSOLIDATE_MODEL` (defaults to `LLM_MODEL`; a strong model like gpt-4.1 is needed — a small model breaks merge atomicity).
