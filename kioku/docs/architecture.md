...
│   │   │   ├── ingest/             # transcript → atomic facts (consolidate, append, curate, sessions, summary, parser)
...
│   │   │   ├── maintenance/        # in-process consolidation cron (scheduler.ts)
...
│   │   ├── prompts/                # extraction.md (8K) + answer.md (3K) + curate.md + consolidate.md
│   │   ├── tests/                  # vitest + mongodb-memory-server
│   │   ├── scripts/                # curate CLI, bench worker + consolidation gate, BM25 probe
│   │   └── bench/longmemeval/      # benchmark harness + datasets + results
│   └── dashboard/                  # Next.js 16 inspector (https://kioku.localhost)
...
- **Atomic facts are write-once on ingest.** The ingest path never UPDATEs / DELETEs `facts`. Corrections happen by appending newer facts with later `event_date`; the answerer prompt resolves contradictions newest-wins (`prompts/answer.md`). The sanctioned mutation paths are the operator-run curation pass and the consolidation cron (both below). Every mutation leaves a row in `history` for postmortem.
- **Periodic durable-only consolidation (in-process).** Beyond the operator-run curation pass (`ingest/curate.ts` via `scripts/curate.ts`), an optional in-process timer (`maintenance/scheduler.ts`, gated on `KIOKU_CONSOLIDATE_ENABLED`, **default off**) runs a single entity-grouped durable-only pass (`prompts/consolidate.md`) over the default scope on an interval — DROPPING episodic chat-exhaust and folding a fragmented episode into one durable fact. Single pass by design: a converging multi-round variant was tried to also clean up the benign cross-group duplicates one entity pass leaves, but the LongMemEval gate showed it over-consolidates (55% dropped vs 42%, 77%→66% recall), so it was dropped (see [bench.md](bench.md)). Same `history`-journaled mutation path as the curation pass (actor `consolidate-cron`); merge categories are clamped to the fixed enum. A guarded `POST /consolidate` triggers one pass manually when enabled. The editor model is `KIOKU_CONSOLIDATE_MODEL` (defaults to `LLM_MODEL`; a strong model like gpt-4.1 is needed — a small model breaks merge atomicity).
...
- **Pull-only by design (system level).** Kioku exposes an API; it never initiates outbound calls to sibling services in the Kagami workspace. Its only outbound network calls are to the configured LLM and embedding endpoints. The in-process consolidation cron is internal maintenance over the local store, not an outbound call, so it does not break this posture.
...
| `apps/api/src/llm.ts`         | Env resolution (canonical keys + legacy profile shim) and `@kagami/llm` `createInference` wiring; exports `model`, `consolidationModel` (the `KIOKU_CONSOLIDATE_MODEL` editor), `getEmbeddingModel`, `embedQuestion`, `embedTexts`, `llmEndpoint`, `embeddingEndpoint`, `embeddingModelName`. |
| `apps/api/src/maintenance/`   | `scheduler.ts` — the in-process consolidation cron (`startConsolidationScheduler`, env-gated, shutdown-aware) and the serialized `runConsolidationOnce` shared by the timer and `POST /consolidate`.                                                                                          |
| `apps/api/prompts/`           | `extraction.md` (ingest), `answer.md` (answerer), `curate.md` (conservative operator curation), `consolidate.md` (durable-only consolidation). Read at runtime and cached after first read.                                                                                                   |
| `apps/api/scripts/`           | One-off tools: `curate.ts` (operator curation/consolidation CLI); `longmemeval.ts` + worker (bench); `bench-consolidate-{vault,all}.ts` (the consolidation recall gate); `probe-bm25-scores.ts` (BM25 sigmoid calibration); `citation-recall.ts` + `variance-probe.ts` (recall probes).       |
| `apps/api/bench/longmemeval/` | LongMemEval runner + datasets + results. See [bench.md](bench.md).                                                                                                                                                                                                                            |
| `apps/dashboard/src/`         | Next.js 16 App Router inspector. See [dashboard.md](dashboard.md).                                                                                                                                                                                                                            |

## Cross-cutting Concerns

- **Logging.** `apps/api/src/logger.ts` is a thin wrapper around `@kagami/logger`'s `createLogger`, which provides stable `service`/`component`/`env` bindings, ECS / OTel field names, an `error`-key serializer, TTY/`LOG_PRETTY`-gated console formatting, and the optional fail-open Kansoku shipper when `KANSOKU_URL` + `KANSOKU_INGEST_TOKEN` are set. There is no secret/PII redaction today (local-trust only). `pino-http` is mounted as middleware so every request gets a `req.log`. The `pino-http` instance is configured (not default): `/health` probes are not logged at all (`autoLogging.ignore`); the completion line's level is by outcome — `info` for <400, `warn` for 4xx, `silent` for ≥500 (the error handler and `mcp.ts` already log every 5xx with the full stack + context, so the completion line would be a duplicate), and `error` only for pino-http's own transport-level errors; and the `req`/`res` serializers are trimmed to `{ method, url, id }` / `{ statusCode }` instead of dumping every header.
