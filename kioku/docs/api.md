...
// Mounted only when KIOKU_CONSOLIDATE_ENABLED — no unauthenticated
// destructive endpoint exists by default (Kioku has no auth layer).
if (loadEnv().KIOKU_CONSOLIDATE_ENABLED) app.use("/consolidate", consolidateRouter);
```
...
| GET    | `/facts/:id/history` | —                                                                                              | `{ id, events: HistoryEvent[] }` (newest first; ADD from ingest, UPDATE/DELETE from the curation pass / consolidation cron, actors `curate` / `consolidate-cron`)                                                                                                   |
...
| POST   | `/consolidate`       | — (gated: only mounted when `KIOKU_CONSOLIDATE_ENABLED`)                                       | `202 { status: "started" }` — fire-and-forget; kicks off one durable-only converging consolidation pass over the default scope (outcome logged / shipped to Kansoku). `409 { status: "busy" }` if a pass is already in flight (shares the cron's process-wide lock) |
