...
| POST   | `/consolidate`       | — (gated: only mounted when `KIOKU_CONSOLIDATE_ENABLED`)                                       | `202 { status: "started" }` — fire-and-forget; kicks off one durable-only consolidation pass over the default scope (outcome logged / shipped to Kansoku). `409 { status: "busy" }` if a pass is already in flight (shares the cron's process-wide lock) |
