...
│   │   │   ├── routes/      # per-resource Express routers (facts, recall, query, sessions, consolidate, meta, filters)
│   │   │   ├── storage/     # mongo singleton, idempotent indexes, facts, entities, transcripts, history
│   │   │   ├── maintenance/ # in-process consolidation cron (scheduler.ts, env-gated, default off)
│   │   │   ├── server.ts    # express bootstrap + ensureIndexes + consolidation cron + graceful shutdown
...
│   │   │   ├── curate.md       # curation prompt (conservative operator drop/merge/rewrite pass)
│   │   │   └── consolidate.md  # consolidation prompt (durable-only: drops episodic chat-exhaust)
│   │   ├── tests/           # vitest suite + mongodb-memory-server harness
│   │   ├── scripts/         # longmemeval(+worker), bench-consolidate-{vault,all}, citation-recall, probe-bm25-scores, variance-probe, curate
...
- **Atomic facts are write-once on ingest** — the ingest path never UPDATEs/DELETEs the `facts` collection. Corrections happen by appending newer facts with later `event_date`; the answerer prompt resolves contradictions newest-wins. The two sanctioned mutation paths are the operator-run curation pass (`ingest/curate.ts` via `scripts/curate.ts`) and the in-process consolidation cron (`maintenance/scheduler.ts`, default off): LLM-judged drop/merge/rewrite of accreted noise, every mutation journaled as an UPDATE/DELETE row in `history` (actor `curate` or `consolidate-cron`). Every mutation leaves a row in `history`.
...
| Curate the live store (LLM drop/merge/rewrite of noisy facts)  | `apps/api/src/ingest/curate.ts` + `prompts/curate.md`; operator CLI `npx tsx apps/api/scripts/curate.ts` (dry run by default, `--apply` to execute). `--mode entity` groups review by shared entity; `--policy consolidate` (+ `prompts/consolidate.md`) is the durable-only pass that DROPS chat-exhaust. `--apply` with either is a single pass (a converging multi-round variant was tried but over-consolidated — see bench.md). Use `LLM_MODEL=gpt-4.1` (a small model breaks merge atomicity) |
| Periodic consolidation cron (durable-only, in-process)         | `apps/api/src/maintenance/scheduler.ts` (`startConsolidationScheduler` + serialized `runConsolidationOnce`) wired in `server.ts`; `apps/api/src/routes/consolidate.ts` (guarded `POST /consolidate`, mounted only when enabled). Env: `KIOKU_CONSOLIDATE_ENABLED` (default off), `KIOKU_CONSOLIDATE_INTERVAL_HOURS` (24), `KIOKU_CONSOLIDATE_MODEL` (editor override; defaults to `LLM_MODEL`). Editor model resolved in `llm.ts` (`consolidationModel`)                                            |
