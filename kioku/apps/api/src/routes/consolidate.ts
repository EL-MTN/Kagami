...
//
// Fire-and-forget: a full consolidation pass makes many LLM review calls and
// can run for minutes, so we kick it off and return 202 rather than holding
// the connection open. The outcome is logged (and shipped to Kansoku). 409 when
// a pass is already in flight — the run shares the scheduler's process-wide
// lock, so a request that slips past this check still no-ops rather than racing.
