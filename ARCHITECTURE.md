...
| Kokoro  | `TELEGRAM_BOT_TOKEN`, `MONGODB_URI`, `KIOKU_URL` (→ `https://api.kioku.localhost`), `KIZUNA_URL` (→ `https://api.kizuna.localhost`), `KIZUNA_ENABLED`, `LLM_PROVIDER`/`LLM_MODEL`, provider API keys, `KAO_URL` + `KAO_TOKEN` (Google services vended by Kao — both or neither), `MCP_SERVERS` (optional JSON array of external MCP servers Kokoro mounts as a client; empty/unset = none), `ROUTINE_PROPOSALS_ENABLED` (default on — lets Mashiro offer to save a finished task as a routine), `ROUTINE_PROPOSAL_COOLDOWN_DAYS` (default 14 — base quiet window after a declined proposal)                                                                                          |
