...
> **Status note (Kao):** **Identity consolidation is complete.** Both consumers — Kokoro and Kizuna — now fetch Google access tokens from Kao at runtime; neither owns a refresh token anymore. Kao is the single owner of the encrypted refresh tokens (one per named grant) and the consent flow.
...
└──────────────────────────────────────────────────────────────────────┘
      ...
      │  MongoDB (encrypted tokens)  │      Kokoro + Kizuna wired.
      └──────────────────────────────┘
```
...
| Kizuna → Kao                      | runtime HTTP dependency (**live**) | `GET ${KAO_URL}/grants/kizuna/token` with bearer `KAO_TOKEN` via `tracedFetch`; thin in-process access-token cache in `apps/api/src/lib/kao-client.ts` (30 s buffer matching Kao's own, plus in-flight de-dup so a scheduler tick that runs Gmail + Calendar back-to-back issues one vend). Replaced the prior encrypted-Mongo `OAuthToken` model + `/oauth/google/*` web-flow surface; `KIZUNA_OAUTH_ENCRYPTION_KEY` and `GOOGLE_OAUTH_*` envs are gone. The `OAuthError` taxonomy (`no_grant`/`invalid_grant`/`refresh_failed`) is preserved so the ingest workers' pause-on-`invalid_grant` branch is unchanged. `force: true` (used by the sync route's force-run-after-pause path) clears the local cache AND sends `?force=1` to Kao so both caches drop the dead token after a re-consent. Kioku has **no** Google OAuth and never did. |
...
The same convention extends to inter-service config: Kizuna's dashboard reaches its API via `KIZUNA_API_URL=https://api.kizuna.localhost`, Kizuna's `KAO_URL` defaults to `https://api.kao.localhost`, Kokoro's `KIOKU_URL` defaults to `https://api.kioku.localhost`, and Kokoro's `KIZUNA_URL` defaults to `https://api.kizuna.localhost` (Zod defaults in `kokoro/packages/shared/src/config.ts`). The numeric-port forms only matter when running a project standalone outside Portless.
...
**Endpoints.** Both apps run under Portless: API at `https://api.kizuna.localhost`, dashboard at `https://kizuna.localhost`. Portless injects each process's `PORT` and proxies the named HTTPS URLs to those ephemeral ports; the API's `3000` fallback only matters when running it standalone outside Portless. The Google consent redirect URI no longer lands on Kizuna at all — it lands on Kao (`${KAO_PUBLIC_URL}/oauth/callback`), since Kao owns the OAuth flow for the `kizuna` grant.

**HTTP API surface.** Open at single-user localhost — no bearer auth on resource routes. There is no `/oauth/*` surface on Kizuna anymore; consent + grant status live in Kao.
...
| OAuth         | none — consent delegated to Kao at `${KAO_URL}/oauth/kizuna/start`                 |

**Storage.** Mongoose models: `Person`, `Interaction`, `Followup`, `Organization`, `SyncState`. Text indexes on people/interactions; unique sourceRef per Gmail/Calendar id. No `OAuthToken` model — refresh tokens live only inside Kao.

**External services.** Google Gmail API (`gmail.readonly`) and Calendar API (`calendar.readonly`); access tokens vended on demand from Kao. No `google-auth-library` dep, no LLM, no queue, no webhooks.

**Auth model.** Single-user localhost; the OS user is the trust boundary. No API auth, no dashboard login. `USER_EMAILS` is used by the ingest workers to identify which inbox addresses count as "self" (not for auth). The only credential Kizuna holds in-process is `KAO_TOKEN`, the bearer it presents to Kao's `/grants/kizuna/token`. See `kizuna/docs/auth.md` for the threat model.

**Coupling notes.** Kizuna has zero outbound references to Kioku or Kokoro. Its only outbound calls are to Google (Gmail/Calendar) and Kao (token vend). Kokoro consumes Kizuna's API over HTTP when `KIZUNA_ENABLED` is true.
...
**Status.** **Both consumers migrated.** Kokoro fetches Google access tokens from `${KAO_URL}/grants/kokoro/token` via `kokoro/apps/bot/src/services/kao-client.ts`; Kizuna fetches `${KAO_URL}/grants/kizuna/token` via `kizuna/apps/api/src/lib/kao-client.ts`. Neither owns a refresh token anymore — the only previously-plaintext Google credential (Kokoro's `GOOGLE_OAUTH_REFRESH_TOKEN`) and the previously-encrypted-Mongo one (Kizuna's `OAuthToken` model + `KIZUNA_OAUTH_ENCRYPTION_KEY`) are both gone. The identity-consolidation work flagged in earlier revisions of this doc is complete.
...
| Kizuna  | `MONGODB_URI`, `USER_EMAILS`, `KIZUNA_API_URL` (→ `https://api.kizuna.localhost`), `KAO_URL` + `KAO_TOKEN` (Google access vended by Kao — both or neither; without `KAO_TOKEN`, ingest surfaces `refresh_failed`), `KIZUNA_HOST` (standalone fallback). Dashboard env also reads `KAO_URL` to build the "Grant / re-consent in Kao" link on `/sync`.                                                                                                                                                                                                                                                                                                                                 |
...
- **Identity consolidation is complete.** Both Kokoro and Kizuna now vend short-lived Google access tokens from Kao; neither owns a refresh token. The previously-plaintext (`GOOGLE_OAUTH_REFRESH_TOKEN`) and previously-encrypted-Mongo (`OAuthToken` + `KIZUNA_OAUTH_ENCRYPTION_KEY`) Google credentials are both gone from the workspace. **Kioku has no Google OAuth** — it is not and never has been a Google-OAuth consumer.
