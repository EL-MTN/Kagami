...
- `KANSOKU_ALERT_WEBHOOK_URL` now fires a second payload, `kansoku.error.spike`, when a **known** fingerprint hits `KANSOKU_SPIKE_THRESHOLD` (default 10, floor 2) occurrences inside `KANSOKU_SPIKE_WINDOW_MINUTES` (default 5). Re-fires are gated by `KANSOKU_SPIKE_COOLDOWN_MINUTES` (default 60) so a sustained outage doesn't pager-storm. State (`windowStart`/`windowCount`/`lastSpikeAlertAt`) lives on each `errors` doc, projection-stripped from `GET /v1/errors` so the wire shape is unchanged for the dashboard/CLI. Three guards keep the alert tight: (1) `evaluateSpike` short-circuits before any Mongo write when no webhook URL is configured (the spike vars are truly no-ops without alerts), (2) logs whose `ts` is older than the window are skipped — a replay/backfill arriving at wall-clock now doesn't trip spike, and (3) `recordErrors` groups incoming docs by fingerprint and issues one `updateOne` per group, so a brand-new fingerprint with N copies in one batch fires the new-error alert once and never trips spike from its own first sighting. Window roll is an aggregation-pipeline `findOneAndUpdate` with a hoisted `__reset` predicate (no duplicated boolean); cooldown is claimed by a conditional `updateOne` that checks `matchedCount === 0` for the "filter missed" semantic. Both alert kinds share the webhook URL and the fail-open posture (5 s timeout with `timer.unref()` so a shutdown isn't blocked; errors swallowed). The shared positive-integer env parser lives in `lib/env.ts` (also used by `cardinality.ts` and `indexes.ts`).
