...
`executeCode` is also dispatch-only, but for a different reason: the user must review the **program body**, not a summary. The dedicated `executeCode` tool (`apps/bot/src/ai/tools/execute-code.ts`, registered only when `EXECUTE_CODE_ENABLED` is set) raises the confirmation itself via `raisePendingConfirmation` with the full code as a fenced block in `promptText` (preview capped at 3000 chars; the action args always carry the complete code). If `executeCode` lived in `GATED_TOOL_NAMES`, the model could route it through `requestConfirmation` and the user would approve a ≤400-char `summary` without ever seeing the code. On approve, the dispatcher re-validates the args (language enum + 8000-char cap), logs only `{ language, codeLength }` — never the code body, so nothing pasted into a script reaches Kansoku — and runs it in the ephemeral Docker sandbox (`apps/bot/src/services/code-sandbox.ts`). There is no decline store: a denied run is just dropped.
