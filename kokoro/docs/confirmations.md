...
Some actions should be executable through the approval rail but **not** selectable by the model via `requestConfirmation`. `createRoutine` (self-authored routines) and `createSkill` (self-authored procedural context) live in `DISPATCH_ONLY_TOOL_NAMES`, not `GATED_TOOL_NAMES`, so they are **absent from `requestConfirmation`'s enum**. The model cannot raise them directly and bypass the proposal-specific anti-nag guards; it must go through `proposeRoutine` or `proposeSkill` → `raisePendingConfirmation` → approve. `raisePendingConfirmation` is the single rail writer extracted from `requestConfirmation` so both paths share identical create → send → `setPromptMessageId` plumbing; proposals pass a shorter `ttlMs`, a `promptText` that shows the full proposed artifact, and a shared one-pending guard so routine and skill proposal bubbles do not stack. On deny/cancel, the platform callback calls `recordProposalDeclineFromConfirmation(row)`, which discriminates on `action.tool` (not origin, so a routine-raised gated action never trips it) and records the decline in the routine or skill proposal store. See [ai-layer.md](ai-layer.md#proposeroutine-live-conversational-turns-only) and [skills.md](skills.md).
