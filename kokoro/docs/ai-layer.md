...
- **Read tools** (called directly):
  ...
  - `listMyFollowups({ direction?, status?, limit? })` lists owner-relative (`i_owe`/`they_owe`) followups in due-priority order.
...
- **Parameters**: `{ language: "python" | "node", code: string (≤3000 chars — sized so the full program always fits in the approval bubble), description: string (one sentence for the approval bubble) }`
...
- **Workspace mount (`useWorkspace: true`)**: the run gets the persistent workspace mounted read-write at `/workspace` (also the workdir). The container NEVER touches canonical storage — `apps/bot/src/services/workspace-sandbox.ts` materializes an ephemeral copy (chmod 0777/0666: Docker Desktop's VirtioFS uid-maps so macOS doesn't need it, but a Linux host's 65534 user does), bind-mounts it, then manifest-diffs the directory after the run and syncs changes back through the normal workspace write path (quotas enforced; symlinks and invalid paths skipped and reported; deletions become workspace soft-deletes). Synced files carry `source: "sandbox"`. A torn run (timeout / OOM / output overflow) is NOT synced — the canonical store stays at the pre-run state and the summary says "workspace changes discarded"; a non-zero exit DOES sync (writing results then `sys.exit(1)` is legitimate). Workspace runs are serialized by an in-process lock so two can't race the materialize→sync window. The bubble's first line states "with workspace access" — the mount is part of what the user approves. The file delta ("wrote out.csv (12.0 KB); modified …") rides on both the summary and `resultText`. Spans: `code.workspace.materialize` / `code.execute` / `code.workspace.sync`.
- **Logging**: the code body is **never logged** — only `{ language, codeLength, useWorkspace }` — so secrets pasted into a script can't reach Kansoku. The full code persists only in the `PendingConfirmation` row (24 h TTL). The run is spanned as `code.execute`.
