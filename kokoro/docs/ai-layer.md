...
### proposeRoutine (conditional — `ROUTINE_PROPOSALS_ENABLED`, default on; live conversational turns only)
...
### proposeRoutineRefinement (conditional — `ROUTINE_PROPOSALS_ENABLED`, default on; live conversational turns only)
...
- **The outcome signal**: every routine run writes a `RoutineLog` (`status`/`summary`). `getRoutineHealth(chatId)` (`packages/db/src/models/routine.ts`) aggregates the last N runs per routine into **facts only** — `failedRuns` / `emptyRuns` / `noReportRuns` / `lastError` — excluding `trigger: "routine"` sub-runs and in-flight `running` rows. `assembleRoutineContext` annotates the **Available Routines** list with a `⚠ failing — N of last M runs` marker on conversational turns so the model can notice. The display threshold (`routineHealthNote` in `context-assembler.ts`) only decides whether to _show_ the marker; the decision to actually refine stays with the LLM ("all LLM or no LLM").
- **Parameters**: `{ routineId, newPrompt, rationale, newParameters? }`
- **Returns**: `{ proposed: true, confirmationId }` or `{ proposed: false, reason }`
- **Behavior**: Loads the routine, skips if missing/disabled or the prompt is unchanged, computes a `signature` (`refine:{routineId}#{baseVersion}#{promptHash}`), runs the **same durable anti-nag guard** as `proposeRoutine` (plus a one-refinement-per-routine-in-flight check), then raises a tap-to-approve bubble showing the **current → proposed** prompt diff and the rationale. The approved action is the dispatch-only `updateRoutinePrompt`.
- **`updateRoutinePrompt` is dispatch-only**: absent from `GATED_TOOL_NAMES`, dispatched only through the rail. At approve time it re-validates against `updateRoutinePromptArgs` — which omits `purity`/`cronSchedule`/`reportMode`/`enabled`, so **a refinement can only rewrite the prompt (and parameters), never escalate a read routine to action or add a schedule**. A **staleness guard** rejects the update if `routine.version !== baseVersion` (the routine changed in the ≤2h the bubble sat), so a stale proposal can't clobber a newer edit. Success bumps `version` (auditable/reversible via the dashboard) and records an `accepted`; deny/cancel records a `declined`.
- **Offered on live conversational turns only**: registered alongside `proposeRoutine` under `ROUTINE_PROPOSALS_ENABLED && ctx.conversational` — same structural exclusions (watchers, routine executions, proactive). A routine can't self-edit routines.
  **Architecture**: Tool in `apps/bot/src/ai/tools/routine-refinements.ts` (`createProposeRoutineRefinementTool`, `computeRefinementSignature`, and the shared cores `proposeRefinement` / `proposeRetirement`). `updateRoutinePrompt` dispatch case + the extended `recordProposalDeclineFromConfirmation` in `apps/bot/src/services/gated-actions.ts`. Health helper `getRoutineHealth` + the shared `NO_REPORT_SENTINEL` in `packages/db/src/models/routine.ts`. Prompt annotation in `apps/bot/src/ai/context-assembler.ts`; system-prompt rule in `apps/bot/context/instructions/routine-refinement.md`. Reuses the `RoutineProposalDecision` store and the confirmation rail.

### Automated self-review pass (conditional — `ROUTINE_PROPOSALS_ENABLED`, on whenever proposals are)
...
- **Flow** (`apps/bot/src/services/routine-review.ts`): for every chat with enabled routines (`listChatIdsWithRoutines`), `getRoutineHealth` is computed and a **mechanical pre-filter** (`needsReview` — ≥4 runs and ≥50% failed/empty; facts only) selects candidates. Each candidate gets one constrained `generateObject` pass (`ModelTier.Smart`, tracked under the new `routine-review` usage category) returning `{ action: "refine" | "retire" | "none", newPrompt?, rationale }`. `refine` → `proposeRefinement`; `retire` → `proposeRetirement`; `none` → nothing. The LLM owns the decision; the pre-filter only bounds cost.
- **Guardrails**: every proposal still passes the durable anti-nag guard (a routine the user already declined stays quiet) and the one-proposal-per-routine-in-flight check; a per-run cap (`MAX_PROPOSALS_PER_RUN = 2`) stops a flood and bounds LLM spend, logging what it defers. The pass is gated by `ROUTINE_PROPOSALS_ENABLED` — no separate flag.
- **`disableRoutine` is dispatch-only**: retirement **disables** (reversible via `manageRoutines` enable / the dashboard), never deletes (deleting would also wipe `RoutineLog` history). Same `baseVersion` staleness guard as `updateRoutinePrompt`, so a routine edited (perhaps fixed) since the bubble was raised isn't disabled out from under that edit.
