...
Time awareness is **split by how often each piece changes**, so the precise clock never busts the provider's prompt cache (the cache is a prefix match; render order is `tools → system → messages`, so a per-message value anywhere in the system prefix invalidates the whole downstream prefix every turn — see `shared/prompt-caching.md`). All helpers live in `apps/bot/src/ai/prompts.ts` and template against `config.TIMEZONE`:
