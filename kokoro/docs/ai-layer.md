...
- **Guardrails**: every proposal still passes the durable anti-nag guard (a routine the user already declined stays quiet) and the one-proposal-per-routine-in-flight check; a per-run cap (`MAX_PROPOSALS_PER_RUN = 2`) stops a flood and bounds LLM spend, logging what it defers. The pass is always on — no feature flag.
