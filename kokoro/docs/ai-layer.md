...
- **Flow** (`apps/bot/src/services/routine-review.ts`): for every chat with enabled routines (`listChatIdsWithRoutines`), `getRoutineHealth` is computed and the shared `routineNeedsAttention` pre-filter (re-exported as `needsReview`) selects candidates. Each candidate gets one constrained `generateObject` pass (`ModelTier.Smart`, capped at 4000 chars, tracked under the new `routine-review` usage category) returning `{ action: "refine" | "retire" | "none", newPrompt?, rationale }`. `refine` → `proposeRefinement`; `retire` → `proposeRetirement`; `none` → nothing. The LLM owns the decision; the pre-filter only bounds cost.
