...
- Kizuna CRM tools — `findPeople`, `getPersonContext`, `recentInteractions`, and `listMyFollowups`, always registered. These tools use the GET-only `@kokoro/kizuna` client and return compact projections, so watchers can observe relationship context without creating people, interactions, or followups.
