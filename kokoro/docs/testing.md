...
| `models/skill.ts`                | ✅     | `tests/models/skill.test.ts`                | CRUD + per-chat unique names, `updateSkillIfVersion` CAS (version/chat/enabled guards), review-stamp invalidation on version bumps (CAS + dashboard-style `updateSkill`; enabled-only toggles keep the stamp), `skillNeedsReview` staleness/cooldown matrix, `markSkillsReviewed` (no `updatedAt`/`version` bump), `listChatIdsWithSkills`. |
...
| `services/skill-review.ts`                                                                                           | ✅     | `tests/services/skill-review.test.ts`       | Weekly skill curation pass: facts-only candidate selection + ordering + cap, one-LLM-call-per-chat, refine/archive/merge routing, malformed-action fall-through, stop-after-first-proposal, blank-metadata normalization, disposition-aware `lastReviewedAt` stamping (no-action verdicts stamp; cap-deferred / pending-suppressed / thrown candidates stay unstamped; nothing on LLM failure).                                                                                                                                                                                                                                                                                                                                                                                   |
