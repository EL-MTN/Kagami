...
| `models/skill.ts`                | ✅     | `tests/models/skill.test.ts`                | CRUD + per-chat unique names, `updateSkillIfVersion` CAS (version/chat/enabled guards), review-stamp invalidation on version bumps (CAS + dashboard-style `updateSkill`; enabled-only toggles keep the stamp), `skillNeedsReview` staleness/cooldown matrix, `markSkillsReviewed` (no `updatedAt`/`version` bump; version-conditional — skips a skill edited mid-pass), `listChatIdsWithSkills`. |
| `models/skill-proposal.ts`       | ✅     | `tests/models/skill-proposal.test.ts`       | Anti-nag decline store: one row per chat/signature, deny-count escalation, quiet-window expiry.                                                                                                                                                                                                                                                                                                  |
...
| `services/gated-actions.ts`                                                                                          | ✅     | `tests/services/gated-actions.test.ts`      | sendEmail, manageCalendar (update + delete), browseAgent (happy + truncation + Target/Browser-closed reset path), dispatch-only skill curation (`updateSkill`/`disableSkill`/`mergeSkills`: CAS, version_conflict/not_found/state_conflict, already-archived no-op, duplicate-absorbee rejection at the args schema, absorbee preflight cancels before the survivor write, survivor-first merge order, partial_merge as residual-race backstop), decline routing to the routine vs skill proposal store.                                                                                                                                                                                                                                                                                                    |
...
| `services/gmail.ts`                                                                                                  | ✅     | `tests/services/gmail.test.ts`              | Gmail client behavior with mocked downstream.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `services/kao-client.ts`                                                                                             | ✅     | `tests/services/kao-client.test.ts`         | Google-token vend client: cache buffer, inflight de-dup, `?force=1` retry, error taxonomy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `services/mcp.ts`                                                                                                    | ✅     | `tests/services/mcp.test.ts`                | External MCP client mount: tool namespacing, fail-open per server.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `services/routine-review.ts`                                                                                         | ✅     | `tests/services/routine-review.test.ts`     | Weekly routine self-review pass: candidate set, grade → action, regression revert.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `services/skill-review.ts`                                                                                           | ✅     | `tests/services/skill-review.test.ts`       | Weekly skill curation pass: facts-only candidate selection + ordering + cap, one-LLM-call-per-chat, refine/archive/merge routing (incl. absorbName dedupe), malformed-action fall-through, stop-after-first-proposal, blank-metadata normalization, disposition-aware version-conditional `lastReviewedAt` stamping (no-action verdicts stamp; cap-deferred / pending-suppressed / thrown candidates stay unstamped; nothing on LLM failure).                                                                                                                                                                                                                                                                                                                                                               |
...
| `ai/tools/*`                                                                                                         | ✅     | `tests/ai/tools/*.test.ts`                  | Every LLM tool covered. Files map 1:1 to the test files: `browse`, `calendar`, `confirmations`, `crm`, `delegate`, `email`, `media`, `memory`, `routines` (CRUD + `useRoutine` nesting/purity), `routine-proposals`, `routine-refinements`, `skills`, `skill-proposals`, `skill-refinements` (curation cores: version-scoped signatures, patch pruning incl. merge metadata, bubble shows metadata values, merge guards incl. duplicate absorbees), `time`, `watchers`, `web-search`, plus the `index.ts` registry (allTools env-gating, watcherTools read-only invariant, routineToolsUnderWatcher transitive read-only check). Helper leaves (`proposal-guard`, `routine-params`, `routine-schema`, `routine-proposal-tools`, `skill-proposal-tools`) are exercised transitively through their consumers. |
...
| `services/chat-review-runner.ts`                                                                                     | ✅     | `tests/services/chat-review-runner.test.ts` | Shared per-chat review driver: FIFO serialization of overlapping passes (queued pass never starts mid-pass), a failed pass rejects its caller without poisoning the queue, per-chat failure isolation, missing-adapter skip.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `services/{confirmation-events,geocoding,google-auth,google-calendar,location,routine-executor,watcher-executor}.ts` | ❌     | —                                           | Service implementations. Reached transitively by the gated-action and tool-contract tests; the executors are pipeline phase.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `scheduler/review-scheduler.ts`                                                                                      | ✅     | `tests/scheduler/review-scheduler.test.ts`  | Shared periodic-review factory: interval anchored to the startup delay (per-pass stagger survives past boot), stop() at every phase, rejecting runs logged without killing the schedule. Fake timers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `scheduler/{proactive,reminders,routines,routine-review,skill-review,watchers,maintenance}.ts`                       | ❌     | —                                           | Timer-driven loops (the two review schedulers are thin wrappers over the shared `review-scheduler.ts` factory, which is tested). Pipeline phase with `vi.useFakeTimers()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
