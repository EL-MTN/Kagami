...
| Document attachment (anything else)   | Inline `data` decoded — or fetched by GUID via `BlueBubblesClient.downloadAttachment` when not inlined (25 MB cap) — then saved to the workspace `inbox/` with a `[file saved to workspace: …]` marker (`WORKSPACE_ENABLED`; placeholder marker otherwise). Fetch failure degrades to `[file … could not be retrieved]`. |
