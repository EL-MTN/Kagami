...
- **Persistent workspace is global, GridFS-backed** (`WORKSPACE_ENABLED`) — one file tree shared across chats/channels/routines (the Kioku global-memory model, not the chat-scoped skills/routines model): `WorkspaceFile` rows own path→key metadata, the `workspace` bucket owns bytes. Soft-delete trash (30-day purge in daily maintenance); quotas enforced in `apps/bot/src/services/workspace.ts`. See [ai-layer.md](ai-layer.md) for the listFiles/readFile/writeFile/deleteFile tools.
