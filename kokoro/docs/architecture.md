...
| `@kokoro/db`        | MongoDB connection, all models, GridFS               | `connectDB`, `disconnectDB`, `Conversation`, `PendingFact`, `Reminder`, `SchedulerState`, `TokenUsage`, `Routine`, `RoutineLog`, `Skill`, `Watcher`, `WatcherLog`, `LocationHistory`, `PendingConfirmation`, `WorkspaceFile`, `readImage`/`writeImage`, `readAudio`/`writeAudio`, `readWorkspaceBlob`/`writeWorkspaceBlob`, all model CRUD functions |
...
- **Relationship context delegated to Kizuna** — `@kokoro/kizuna` is a GET-only HTTP client for compact CRM projections. It uses `KIZUNA_URL` (default `https://api.kizuna.localhost`) and the CRM tools are always registered. The bot tools fail open with sanitized degraded results if Kizuna is unreachable.
...
- **MongoDB stores deterministic state only** — sessions, reminders, confirmations, routines, skills, watchers, location history, workspace files. Long-term memory lives in Kioku's vault.
- **GridFS image storage** — user-sent photos stored in MongoDB GridFS (`images` bucket) instead of inline base64
- **Persistent workspace is global, GridFS-backed, always on** — one file tree shared across chats/channels/routines (the Kioku global-memory model, not the chat-scoped skills/routines model): `WorkspaceFile` rows own path→key metadata, the `workspace` bucket owns bytes. Soft-delete trash (30-day purge in daily maintenance); quotas (`WORKSPACE_MAX_*`) enforced in `apps/bot/src/services/workspace.ts`. `executeCode` runs can mount it (`useWorkspace`) via an ephemeral materialize → sync-back bridge (`workspace-sandbox.ts`) — the container never touches canonical storage. See [ai-layer.md](ai-layer.md) for the listFiles/readFile/writeFile/deleteFile/sendFile tools.
