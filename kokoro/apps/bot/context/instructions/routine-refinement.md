...
Also check relevant skills with `searchSkills` / `readSkill` when the failure looks like missing style, policy, or project procedure. A good refinement can import that durable guidance into the routine prompt.
