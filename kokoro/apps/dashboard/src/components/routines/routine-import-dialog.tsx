...
import { routineExportBundleSchema, type RoutineExportBundle } from "@/lib/routine-schema";
...
type ImportState =
  | { step: "idle" }
  | { step: "preview"; bundle: RoutineExportBundle }
  | { step: "importing" }
  | { step: "result"; imported: number; skipped: number; errors: string[] };

interface RoutineImportDialogProps {
  onImported: () => void;
}

export function RoutineImportDialog({ onImported }: RoutineImportDialogProps) {
  ...
  function tryParse(text: string) {
    ...
    try {
      ...
      const result = routineExportBundleSchema.safeParse(json);
      if (result.success) {
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="font-display text-xl">Import Routines</DialogTitle>
        </DialogHeader>

        {state.step === "result" ? (
          <div className="space-y-3">
            <p className="text-sm text-foreground/90">
              Imported <strong>{state.imported}</strong> routine
              {state.imported !== 1 ? "s" : ""}
              {state.skipped > 0 && (
                <>
          ...
          <>
            ...
            <Textarea
              ...
              placeholder='{"version": 1, "routines": [...]}'
            ...
            {state.step === "preview" && (
              <div className="space-y-2">
                <p className="text-sm text-foreground/90">
                  Found <strong>{state.bundle.routines.length}</strong> routine
                  {state.bundle.routines.length !== 1 ? "s" : ""}:
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {state.bundle.routines.map((s) => (
                    <span
                      key={s.name}
