...
| Self-improving routines (proposeRoutineRefinement) | `apps/bot/src/ai/tools/routine-refinements.ts` (tool + shared `proposeRefinement`/`proposeRetirement` cores) + `apps/bot/src/services/gated-actions.ts` (dispatch-only `updateRoutinePrompt` + version guard) + `getRoutineHealth`/`NO_REPORT_SENTINEL` in `packages/db/src/models/routine.ts` (outcome signal) + `apps/bot/src/ai/context-assembler.ts` (⚠ health annotation) + `apps/bot/context/instructions/routine-refinement.md` (prompt rule). Reuses the `routine-proposal.ts` decline store. See [docs/ai-layer.md](docs/ai-layer.md) |
| Automated routine self-review (unprompted)         | `apps/bot/src/services/routine-review.ts` (`needsReview` pre-filter + LLM pass → `proposeRefinement`/`proposeRetirement`) + `apps/bot/src/scheduler/routine-review.ts` (weekly, wired in `apps/bot/src/index.ts`) + `apps/bot/src/services/gated-actions.ts` (dispatch-only `disableRoutine`) + `listChatIdsWithRoutines` in `packages/db/src/models/routine.ts`. Gated by `ROUTINE_PROPOSALS_ENABLED`. See [docs/ai-layer.md](docs/ai-layer.md)                                                                                               |
