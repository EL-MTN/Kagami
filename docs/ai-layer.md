...
Implemented in `src/ai/context-assembler.ts`. The system prompt is built from MongoDB and the personality card at generation time.
...
allTools(ctx) → { rememberFact, noteToSelf, readMemory, searchMemory, listMemories, curateMemory, sendPhoto, checkEmail?, manageCalendar?, manageReminders? }
```
