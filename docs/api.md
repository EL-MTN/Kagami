...
| POST   | `/facts`              | `{ text, event_date?, source_session?, user_id?, run_id?, agent_id?, metadata?, category? }`                              | `201 { id, status: "added" }` or `200 { id, status: "duplicate", reason: "hash"|"cosine", similarity? }`  |
...
| POST   | `/sessions`           | `{ transcript, generate_summary?, user_id?, run_id?, agent_id?, metadata? }`                                              | `201 { sessionId, added, batches, summaryFactId }`                                                        |
...
| `ingest_session` | `{ transcript, generate_summary?, user_id?, run_id?, agent_id?, metadata? }`                                          | `{ sessionId, added, batches, summaryFactId }`                                                                                         |
