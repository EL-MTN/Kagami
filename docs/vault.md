...
| `getActiveFollowUps(limit?, maxAgeDays?)`         | Follow-ups with 30-day age limit and dedup. Returns `string[]`.                   |
| `getActiveFollowUpsWithIds(limit?, maxAgeDays?)`  | Follow-ups with memory IDs. Returns `{memoryId, text}[]`. Used by curator.        |
