...
**Key files:** `packages/db/src/models/routine.ts`, `apps/bot/src/services/routine-executor.ts`, `apps/bot/src/ai/tools/use-routine.ts`

### OpenClaw Routines — Prompt injection from markdown files

The runtime scans the workspace for `ROUTINE.md` files, injects a compact `<available_routines>` list into the system prompt. When the model decides a routine is relevant, it reads the full `ROUTINE.md` into context via a file read. The routine's instructions become part of the _same_ LLM call — no sub-agent, no separate execution. Routines are just markdown on disk.

### Claude Code Routines — Lazy-loaded prompt expansion with optional forked subagents

Routine descriptions (from YAML frontmatter) are loaded into context at a 2% budget. When triggered (auto-detected or via `/slash-command`), the `Routine` tool reads `ROUTINE.md` content into the current context window. Optionally, `context: fork` spawns an isolated subagent with its own context. Routines are filesystem directories with `ROUTINE.md` + supporting files + scripts.
...
Claude Code balances structure and simplicity. The frontmatter system (`disable-model-invocation`, `context: fork`, `allowed-tools`, `agent`) gives fine-grained control without complexity. The layered scoping (enterprise > personal > project > plugin) fits professional workflows. Supporting files + scripts let routines bundle real logic. But there's no marketplace — sharing is via git.
...
Claude Code is the most architecturally sophisticated in terms of _execution structure_. The `context: fork` + `agent` system lets you pick execution models per routine (inline for cheap reference, forked `Explore` agent for research, forked `Plan` agent for architecture). The `allowed-tools` restriction is something nobody else has — you can create a read-only routine that literally can't modify files. But parameter handling is primitive (string interpolation only).
...
The term "routine" is overloaded across the agent ecosystem. What each project means:
...
- [Extend Claude with routines](https://code.claude.com/docs/en/routines) — Claude Code Docs
...
- [Inside Claude Code Routines: Structure, prompts, invocation](https://mikhail.io/2025/10/claude-code-routines/) — Mikhail Shilkov
- [Snyk ToxicRoutines Study](https://snyk.io/blog/toxicroutines-malicious-ai-agent-routines-clawhub/) — Snyk
