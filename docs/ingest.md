...
└── sessions.ts          # HTTP-shaped wrapper: parse + upsert transcript + consolidate + summary fact
```
...
```
ingestSessionFromString({ transcript, generate_summary, user_id, run_id, agent_id, metadata })
  ...
  │     generateSummaryClause(turns)              ← short keyword-rich clause via generateObject
  │     appendSingleFact("On <date>, conversation covered: <topics>.", scope)
  │     summaryFactId = result.id                 ← hash dedup short-circuits on re-ingest
  └─ return { sessionId, added, batches, summaryFactId }
```

The summary clause has a different shape and purpose than the narrative session summary fed into extraction:

| Object                                    | Generated by                       | Stored where                                                    | Purpose                                                                            |
| ----------------------------------------- | ---------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| **Narrative summary** (4–8 sentences)     | `generateNarrativeSummary` in `session-summary.ts` | `session_summaries` collection, keyed by `source_session`        | Grounds entities and references for the per-batch extractor.                       |
| **Summary fact** (one keyword-rich clause)| `generateSummaryClause` in `sessions.ts`           | A single `Fact` row in `facts`, hash-dedup'd, source-tagged       | Surfaces in retrieval as a "session-level" hit when a query is broad and topical.  |

Both are skippable: narrative summary returns `""` on failure (extraction proceeds without context); summary fact is gated by `generate_summary !== false`.
