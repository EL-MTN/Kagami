...
4. Output the refresh token to add to your `.env`
...
### Google Auth (`src/services/google-auth.ts`)
...
### Calendar Service (`src/services/google-calendar.ts`)
...
- Three new tools are added to `allTools()` (`src/ai/tools/index.ts`)
- `MAID_SERVICE_INSTRUCTIONS` are injected into the system prompt (`src/ai/context-assembler.ts`)
- The curator formats tool calls for the new tools (`src/memory/curator.ts`)
