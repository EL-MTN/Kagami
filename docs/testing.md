...
- `apps/bot/src/ai/tools/use-skill.ts` — two related fixes in
  `buildParamSchema`:
...
| `skill-validation.ts` | ✅     | `tests/skill-validation.test.ts` | `computeNextRunAt` + every `validateCronAndDefaults` branch.                                          |
...
| `models/skill.ts`                | ✅     | `tests/models/skill.test.ts`                | CRUD, `getDueSkills`, atomic manual-claim, parent-log linkage, log cleanup.                                                |
...
| `ai/tools/*` (21 files including `index.ts`)                                                                                     | ✅     | `tests/ai/tools/*.test.ts`               | Every LLM tool covered: memory tools, service wrappers (send-email/photo/voice, check-email, manage-calendar incl. readOnly), CRUD+lifecycle (manage-skills/watchers/reminders, search-skills, report-watcher-result), nesting + purity gate (use-skill), confirmation primitives (request/cancel-confirmation), browse (full + readOnly), and the `index.ts` registry (allTools env-gating, watcherTools read-only invariant, skillToolsUnderWatcher transitive read-only check). |
...
| `services/{browser,confirmation-events,geocoding,gmail,google-auth,google-calendar,location,skill-executor,watcher-executor}.ts` | ❌     | —                                        | Service implementations. Reached transitively by the gated-action and tool-contract tests; the executors themselves are pipeline phase.                                                                                                                                                                                                                                                                                                                                            |
| `scheduler/{proactive,reminders,skills,watchers}.ts`                                                                             | ❌     | —                                        | Timer-driven loops. Pipeline phase with `vi.useFakeTimers()`.                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- **Pipeline tests.** Real DB, `mockLLM`, fake timers, MSW. Generate-pipeline
  ...
  proactive scheduler timer recovery, watcher and skill executors.
